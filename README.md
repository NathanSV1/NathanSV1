# Nathan Bruyere — Research & Projects

Welcome to my personal research and side project portfolio.  
This repository gathers my analytical work in **finance**, **programmation** and **geopolitical studies** developed during my time as an engineering student & international middle distance athlete at Grenoble INP, France. 

---

## Finance & Quantitative Research

| Project | Description | Format |
|----------|--------------|--------|
|[Mini Backtest Engine](https://github.com/NathanSV1/mini-backtest-engine) | Design of a modular backtesting framework fo multiple mono-asset strategies | Code + Report (EN)|
| [On Market Efficiency (FR)](./finance/market_efficiency) | Discussion sur l'efficience des marchés financiers, d'un point de vu néo-libéral | Rapport |
| [Portfolio optimisation with the Markowitz model (FR) ](./finance/portfolio-optimisation-markowitz) | Discussion sur le modèle d'optimisation du portefeuille de Markowitz| Rapport |
| [Mastermind – Codebreakers & Codemakers](https://github.com/NathanSV1/mastermind) | Implémentation et comparaison de différents algorithmes de résolution du Mastermind| Code + Rapport (FR)|









## Reflections

| Title | Topic |
|-------|--------|
| [Terrorisme : origines et mutations](geopolitics/Terrorisme___origines___mutations.pdf) | _Exploration de l'évolution du terrorisme, de 1793 à 2016_ |

---

>  Feel free to connect: [LinkedIn](www.linkedin.com/in/nathan-bruyere-6066091a2) | [Email](mailto:nathan-bruyere@hotmail.com)

